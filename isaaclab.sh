...
extract_python_exe() {
    ...
    if ! [[ -z "${CONDA_PREFIX}" ]]; then
        ...
        # Check if current python has isaacsim-rl installed (conda env might be active without CONDA_PREFIX)
        local current_python=$(which python 2>/dev/null)
        if [ ! -z "${current_python}" ] && [ $(${current_python} -m pip list 2>/dev/null | grep -c 'isaacsim-rl') -gt 0 ]; then
            local python_exe=${current_python}
        else
            # use kit python
