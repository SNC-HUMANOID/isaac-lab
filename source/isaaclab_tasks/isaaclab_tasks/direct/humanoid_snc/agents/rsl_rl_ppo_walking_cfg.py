...
@configclass
class HumanoidSNCR5WalkingPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 48          # Longer episodes for better gait learning
    max_iterations = 2500           # Sufficient iterations with better parameters
    save_interval = 50              # Save more frequently for monitoring
    experiment_name = "humanoid_snc_r5_walking_v2"
    empirical_normalization = False
    
    policy = RslRlPpoActorCriticCfg(
        init_noise_std=0.2,         # Lower initial exploration for stability
        actor_hidden_dims=[512, 256, 128],  # Simpler network for more stable learning
    ...
    algorithm = RslRlPpoAlgorithmCfg(
        ...
        clip_param=0.15,            # Smaller clip for more conservative updates
        entropy_coef=0.005,         # Lower entropy for more deterministic actions
        num_learning_epochs=6,      # Moderate learning epochs
        num_mini_batches=6,         # Moderate mini-batches
        learning_rate=1.5e-4,       # Lower learning rate for stable walking
        schedule="adaptive",
        gamma=0.99,                 # Standard gamma
        lam=0.95,                   # Standard lambda
        desired_kl=0.01,            # Standard KL divergence
