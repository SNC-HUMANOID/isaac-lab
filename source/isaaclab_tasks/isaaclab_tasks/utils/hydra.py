...
def hydra_task_config(task_name: str, agent_cfg_entry_point: str) -> Callable:
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            # Handle case where task_name might be None (not passed via CLI)
            # In this case, we expect it to be the first Hydra argument
            import sys
            resolved_task_name = task_name
            if resolved_task_name is None and len(sys.argv) > 1:
                resolved_task_name = sys.argv[1]
            
            if resolved_task_name is None:
                raise ValueError("Task name must be provided either via --task CLI argument or as first positional argument")
            
            # register the task to Hydra
            env_cfg, agent_cfg = register_task_to_hydra(resolved_task_name.split(":")[-1], agent_cfg_entry_point)

            # define the new Hydra main function
            @hydra.main(config_path=None, config_name=resolved_task_name.split(":")[-1], version_base="1.3")
